...
// Оптимизация: Используем только latin для главного шрифта (улучшает LCP)
const geistSans = Geist({
  variable: "--font-geist-sans",
  subsets: ["latin"],
  display: "swap", // Попеременное отображение - быстрее показывает текст
});
...
export const metadata: Metadata = {
  ...
  // Оптимизация: Указываем метатеги для улучшения сер исо
  viewport: {
    width: 'device-width',
    initialScale: 1,
    maximumScale: 5,
...
export default function RootLayout({
  ...
  return (
    <html lang="ru">
      {/* Оптимизация LCP: Добавляем preconnect к критичным ресурсам */}
      <head>
        <link rel="preconnect" href="https://fonts.googleapis.com" />
        <link rel="preconnect" href="https://fonts.gstatic.com" crossOrigin="anonymous" />
        <link rel="dns-prefetch" href="https://mc.yandex.ru" />
        <link rel="dns-prefetch" href="https://cdn.callibri.ru" />
        {/* Оптимизация: Предзагрузка критичных CSS */}
        <link rel="preload" as="style" href="../styles/globals.css" />
      </head>
      <body className={`${geistSans.variable} ${geistMono.variable} antialiased`}>
        {children}

        {/* 💬 Кнопка WhatsApp - Оптимизирована */}
        <a href="https://wa.me/79852707575" target="_blank" className="whatsapp" rel="noopener noreferrer">
        ...
        {/* 🔥 Яндекс.Метрика - Оптимизирована (загружается после интерактивности) */}
        <Script
          id="yandex-metrika"
        ...
        <noscript>
          <div>
            <img
              src="https://mc.yandex.ru/watch/94203012"
        ...
        {/* 📞 Callibri коллтрекинг - загружается после (не блокирует LCP) */}
        <Script
